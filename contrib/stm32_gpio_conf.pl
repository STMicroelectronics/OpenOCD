...
# "Blue-Pill stm32f103cbt6 board w/ msoftspi
#$STM32F1 = 1;
...
# mini-stm32f030f4p6 board w/ msoftspi
#$GPIO_BASE = 0x48000000;
#$Conf = "PA07:PP:V, PA06:INUP:V, PA05:PP:V, PA04:PP:M, PB01:PP:V";

# stm32f407vet6 board w/ msoftspi
...
foreach my $line (@conf)
{
	...
	if ($conf eq '')
	{
		...
		{
			$Exor = sprintf("0x%08X %2d 0x%02X 0x%08X", 0, 0, 0xFF, 0);
			push(@Col, $Exor);
	...
	if ($STM32F1)
	{
		...
		(($MODER & 0x3) == 0x0) && ($Exor .= sprintf(" 0x%02X 0x%08X",
			((($pin >= 8) ? ${GPIO_CRH} : ${GPIO_CRL})-${GPIO_ODR}) & 0xFF, $exor));
	}
	else
	{
		$$out{'DEF'} |= (1 << ${pin});
		...
		$Exor = sprintf("0x%08X %2d", ${GPIO_BASE} + (ord($port)  - ord('A')) * ${GPIO_OFFS} + ${GPIO_ODR}, $pin);
		my $exor = 0x1 << ($pin << 1);
		($MODER == 0x0) && ($Exor .= sprintf(" 0x%02X 0x%08X", (${GPIO_MODER}-${GPIO_ODR}) & 0xFF, $exor));
	}
...
my $Col = sprintf("${Sep}0x%02X ", (${GPIO_IDR}-${GPIO_ODR}) & 0xFF);
for (my $i = 0; $i < @Col; $i++)
{
	if (($i != 0) && (($i % 2) == 0))
	{
