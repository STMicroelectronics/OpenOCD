...
/* flash bank spc582b <base> <size> 0 0 <target#> */
FLASH_BANK_COMMAND_HANDLER(spc582b_flash_bank_command)
{
	struct spc582b_flash_bank *spc582b_info;
	...
	spc582b_info = malloc(sizeof(struct spc582b_flash_bank));
	bank->driver_priv = spc582b_info;

	spc582b_info->probed = 0;
	spc582b_info->user_bank_addr = bank->base;
	spc582b_info->user_bank_size = bank->size;

	return ERROR_OK;
}

static int spc582b_protect_check(struct flash_bank *bank)
{
...
static int spc582b_setlock(struct flash_bank *bank, uint32_t block_space, uint32_t lock_state)
{
	int err;
	struct target *target = bank->target;
	struct spc582b_flash_bank *spc582b_info = bank->driver_priv;
	struct working_area *ssd_config;
	struct working_area *setlock_algorithm;
	struct reg_param reg_params[5];
	struct powerpc_algorithm powerpc_info;
	SSD_CONFIG *ssd = &spc582b_info->ssd;
...
static int spc582b_getlock(struct flash_bank *bank,
...
static int spc582b_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	if (bank->base == 0xFC0000)
	{
		for (i = first; i <= last; i++) {
			if (i < spc582b_info->low_max_index) {
				low_mask |= (1 <<  (i + 1));
			} else if (i < spc582b_info->large_max_index) {
				nLargeBlockSelect.firstLargeBlockSelect |= (1 << (i - spc582b_info->low_max_index));
			} else if (i < spc582b_info->mid_max_index) {
				mid_mask |= (1 << (i - spc582b_info->low_max_index));
			} else if (i < spc582b_info->high_max_index) {
				high_mask |= (1 << (i - spc582b_info->mid_max_index));
			}
		}
	}
	...
	{
		for (i = first; i <= last; i++) {
			if (i < spc582b_info->high_max_index) {
				high_mask |= (1 << i);
			}
		}
	}
	...
	if (low_mask != 0) {
		err = spc582b_getlock(bank, C55_BLOCK_LOW, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc582b_setlock(bank, C55_BLOCK_LOW, (lock_state & 0xFFFFFE00));
		if (err != ERROR_OK)
			return err;
	}

	if (mid_mask != 0) {
		err = spc582b_getlock(bank, C55_BLOCK_MID,  &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc582b_setlock(bank, C55_BLOCK_MID, (lock_state & 0xFFFFFFFC));
		if (err != ERROR_OK)
			return err;
	}

	if (high_mask != 0) {
		err = spc582b_getlock(bank, C55_BLOCK_HIGH, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc582b_setlock(bank, C55_BLOCK_HIGH, (lock_state & 0xFFFFFFF0));
		if (err != ERROR_OK)
			return err;
	}

	if (nLargeBlockSelect.firstLargeBlockSelect != 0) {
		err = spc582b_getlock(bank, C55_BLOCK_LARGE_FIRST, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc582b_setlock(bank, C55_BLOCK_LARGE_FIRST, (lock_state & 0xFFFFFF00));
		if (err != ERROR_OK)
			return err;
...
//	h_u32_to_be((uint8_t *)nLargeBlockSelect_array, nLargeBlockSelect.firstLargeBlockSelect);
//	h_u32_to_be((uint8_t *)&nLargeBlockSelect_array[4], nLargeBlockSelect.secondLargeBlockSelect);
...
static int spc582b_protect(struct flash_bank *bank, int set, unsigned int first, unsigned int last)
{
	struct target *target = bank->target;
...
static int spc582b_write(struct flash_bank *bank, const uint8_t *buffer,
...
static int spc582b_probe(struct flash_bank *bank)
{
	struct spc582b_flash_bank *spc582b_info = bank->driver_priv;
	struct target *target = bank->target;
	struct working_area *ssd_config;
	...
	if (spc582b_info->user_bank_size) {
		LOG_DEBUG("ignoring flash probed value, using configured bank size");
		flash_size_in_kb = spc582b_info->user_bank_size / 1024;
	}

	LOG_INFO("flash: %d kbytes @ 0x%08x", flash_size_in_kb, spc582b_info->user_bank_addr);
	...
	*/
	if(bank->base == 0xFC0000)
	{
		...
		bank->base = spc582b_info->user_bank_addr;
		...
		// Large Flash Blocks
		setup_sector(bank, 8, 6, 128 * 1024);

		spc582b_info->low_max_index = 8;
		spc582b_info->large_max_index = 14;
		spc582b_info->high_max_index = 0;
		spc582b_info->mid_max_index = 0;
	}
	else if(bank->base == 0x800000)
	{
		num_pages = 4;
		...
		spc582b_info->low_max_index = 0;
		spc582b_info->large_max_index = 0;
		spc582b_info->high_max_index = 4;
		spc582b_info->mid_max_index = 0;
	}
...
static int spc582b_auto_probe(struct flash_bank *bank)
{
	struct spc582b_flash_bank *spc582b_info = bank->driver_priv;
	...
	if (spc582b_info->probed)
		return ERROR_OK;
	return spc582b_probe(bank);
}


static int get_spc582b_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	LOG_DEBUG("%s:%d %s()",
		__FILE__, __LINE__, __func__);
...
struct flash_driver spc582b_flash = {
	.name = "spc582b",
	.flash_bank_command = spc582b_flash_bank_command,
	.erase = spc582b_erase,
	.protect = spc582b_protect,
	.write = spc582b_write,
	.read = default_flash_read,
	.probe = spc582b_probe,
	.auto_probe = spc582b_auto_probe,
	.erase_check = default_flash_blank_check,
	.protect_check = spc582b_protect_check,
	.info = get_spc582b_info,
};
