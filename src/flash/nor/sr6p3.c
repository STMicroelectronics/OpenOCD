...
FLASH_BANK_COMMAND_HANDLER(sr6p3_flash_bank_command)
{
	struct sr6p3_flash_bank *sr6p3_info;
	...
	sr6p3_info = malloc(sizeof(struct sr6p3_flash_bank));
	bank->driver_priv = sr6p3_info;
...
static int sr6p3_protect_check(struct flash_bank *bank)
{
...
static int sr6p3_setlock(struct flash_bank *bank, uint32_t block_space, uint32_t lock_state)
{
	int err;
	struct target *target = bank->target;
	struct sr6p3_flash_bank *sr6p3_info = bank->driver_priv;
	struct working_area *ssd_config;
	struct working_area *setlock_algorithm;
	struct reg_param reg_params[5];
	struct armv8_algorithm armv8_algorithm_info;
	SSD_CONFIG *ssd = &sr6p3_info->ssd;
	SSD_CONFIG ssd_lock;
...
static int sr6p3_getlock(struct flash_bank *bank,
...
static int sr6p3_write(struct flash_bank *bank, const uint8_t *buffer,
		uint32_t offset, uint32_t count)
{
	...
	err = sr6p3_getlock(bank, NVM_BLOCK_LOW, &lock_state);
	if (err != ERROR_OK)
		return err;

	err = sr6p3_setlock(bank, NVM_BLOCK_LOW, (lock_state & 0xFFFF0F00));
	if (err != ERROR_OK)
		return err;

	err = sr6p3_getlock(bank, NVM_BLOCK_MID,  &lock_state);
	if (err != ERROR_OK)
		return err;

	err = sr6p3_setlock(bank, NVM_BLOCK_MID, (lock_state & 0xFFFFFF00));
	if (err != ERROR_OK)
		return err;

	err = sr6p3_getlock(bank, NVM_BLOCK_HIGH, &lock_state);
	if (err != ERROR_OK)
		return err;

	err = sr6p3_setlock(bank, NVM_BLOCK_HIGH, (lock_state & 0xFFFFFF00));
	if (err != ERROR_OK)
		return err;
	err = sr6p3_getlock(bank, NVM_BLOCK_256_FIRST, &lock_state);
	if (err != ERROR_OK)
		return err;

	err = sr6p3_setlock(bank, NVM_BLOCK_256_FIRST, (lock_state & 0xF0000000));
	if (err != ERROR_OK)
		return err;

	if (bank->base == 0x29F87000)
	{
		err = sr6p3_getlock(bank, NVM_BLOCK_UTEST,  &lock_state);
		if (err != ERROR_OK)
			return err;
		err = sr6p3_setlock(bank, NVM_BLOCK_UTEST, lock_state & 0xFFFFFFF0);
		if (err != ERROR_OK)
			return err;
	}
...
static int sr6p3_writeToErase(struct flash_bank *bank, const uint8_t *buffer,
		uint32_t offset, uint32_t count)
{
	LOG_INFO("%s:%d %s()", __FILE__, __LINE__, __func__);
	LOG_INFO("%s:%d %s() offset = 0x%08x count = 0x%08x", __FILE__, __LINE__, __func__, offset, count);
...
static int sr6p3_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	if (bank->base == 0x28000000)
	{
		for (i = first; i <= last; i++) {
			if (i < sr6p3_info->low_max_index) {
				low_mask |= (1 <<  (i + 1));
			} else if (i < sr6p3_info->large_max_index) {
				if (i < 13)
				{
					nLargeBlockSelect.first256BlockSelect |= (1 << (i - sr6p3_info->low_max_index));
				}
				else
				{
					nLargeBlockSelect.first256BlockSelect |= (1 << (i - sr6p3_info->low_max_index - 1));
				}
			} else if (i < sr6p3_info->mid_max_index) {
				mid_mask |= (1 << (i - sr6p3_info->low_max_index));
			} else if (i < sr6p3_info->high_max_index) {
				high_mask |= (1 << (i - sr6p3_info->mid_max_index));
			}
		}
	}
	...
	{
		for (i = first; i <= last; i++) {
			if (i < sr6p3_info->mid_max_index) {
				mid_mask |= (1 << i);
			} else if (i < sr6p3_info->large_max_index) {
				if (i < 13)
				{
					nLargeBlockSelect.first256BlockSelect |= (1 << ((i - sr6p3_info->mid_max_index)+14));
				}
				else
				{
					nLargeBlockSelect.first256BlockSelect |= (1 << ((i - sr6p3_info->mid_max_index)+14-1));
				}
			} else if (i < sr6p3_info->low_max_index) {
				low_mask |= (1 << i);
			} else if (i < sr6p3_info->high_max_index) {
				high_mask |= (1 << i);
			}
	...
	else if (bank->base == 0x28900000)
	{
		for (i = first; i <= last; i++) {
			if (i < sr6p3_info->mid_max_index) {
				mid_mask |= (1 << i);
			} else if (i < sr6p3_info->large_max_index) {
				nLargeBlockSelect.first256BlockSelect |= (1 << ((i - sr6p3_info->mid_max_index)+3));
			} else if (i < sr6p3_info->low_max_index) {
				low_mask |= (1 << i);
			} else if (i < sr6p3_info->high_max_index) {
				high_mask |= (1 << i);
	...
	{
		for (i = first; i <= last; i++) {
			if (i < sr6p3_info->high_max_index) {
				high_mask |= (1 << i);
			} else if (i < sr6p3_info->large_max_index) {
				nLargeBlockSelect.first256BlockSelect |= (1 << ((i - sr6p3_info->mid_max_index) + 7));
			} else if (i < sr6p3_info->low_max_index) {
				low_mask |= (1 << i);
			} else if (i < sr6p3_info->mid_max_index) {
				mid_mask |= (1 << i);
...
*/
	...
	for (i=first; i<= last; i++)
	{
		sr6p3_writeToErase(bank, buffer, bank->sectors[i].offset, bank->sectors[i].size);
...
static int sr6p3_protect(struct flash_bank *bank, int set, unsigned int first, unsigned int last)
{
	struct target *target = bank->target;
	uint32_t low_mask = 0;
	uint32_t mid_mask = 0;
	...
	if (low_mask != 0) {
		...
		err = sr6p3_setlock(bank, NVM_BLOCK_LOW, (lock_state & (0xFFFFFFFF & low_mask)));
		if (err != ERROR_OK)
			return err;
	}
	...
	if (mid_mask != 0) {
		...
		err = sr6p3_setlock(bank, NVM_BLOCK_MID, (lock_state & (0xFFFFFFFF & mid_mask)));
		if (err != ERROR_OK)
			return err;
	}
	...
	if (high_mask != 0) {
		...
		err = sr6p3_setlock(bank, NVM_BLOCK_HIGH, (lock_state & (0xFFFFFFFF & high_mask)));
		if (err != ERROR_OK)
			return err;
	}
	...
	if (nLargeBlockSelect.first256BlockSelect != 0) {
		...
		err = sr6p3_setlock(bank, NVM_BLOCK_256_FIRST, (lock_state & (0xFFFFFFFF & nLargeBlockSelect.first256BlockSelect)));
		if (err != ERROR_OK)
			return err;
	}
	...
	if (utest_mask !=0)
	{
		...
		if (err != ERROR_OK)
			return err;
		err = sr6p3_setlock(bank, NVM_BLOCK_UTEST, lock_state & (0xFFFFFFFF & utest_mask));
		if (err != ERROR_OK)
			return err;
	}
...
static int sr6p3_probe(struct flash_bank *bank)
{
	struct sr6p3_flash_bank *sr6p3_info = bank->driver_priv;
	struct target *target = bank->target;
	struct working_area *ssd_config;
	...
	if (sr6p3_info->user_bank_size) {
		LOG_DEBUG("ignoring flash probed value, using configured bank size");
		flash_size_in_kb = sr6p3_info->user_bank_size / 1024;
	}

	LOG_INFO("flash: %d kbytes @ 0x%08x", flash_size_in_kb, sr6p3_info->user_bank_addr);
	...
	*/
	...
	if(bank->base == 0x28000000)   //Cluster 0 core 0
	{
		...
		bank->base = sr6p3_info->user_bank_addr;
		...
		// Large Flash Blocks
		setup_sector(bank, 8, 4, 256 * 1024);
		setup_sector(bank, 12, 2, 128 * 1024);
		setup_sector(bank, 14, 5, 256 * 1024);
		
		sr6p3_info->low_max_index = 8;
		sr6p3_info->large_max_index = 19;
		sr6p3_info->high_max_index = 0;
		sr6p3_info->mid_max_index = 0;
	}
	if(bank->base == 0x28400000)   //Cluster 0 core 1
	{
		...
		sr6p3_info->low_max_index = 0;
		sr6p3_info->large_max_index = 19;
		sr6p3_info->high_max_index = 0;
		sr6p3_info->mid_max_index = 8;
	}
	if(bank->base == 0x28800000)   //Cluster 1 core 0
	{
		...
		// Large Flash Blocks
		setup_sector(bank, 8, 3, 256 * 1024);
		
		sr6p3_info->low_max_index = 8;
		sr6p3_info->large_max_index = 11;
		sr6p3_info->high_max_index = 0;
		sr6p3_info->mid_max_index = 0;
	}
	if(bank->base == 0x28900000)   //Cluster 1 core 0
	{
		num_pages = 11;
	...
	else if(bank->base == 0x29E00000) //Cluster 0 Core 0 Data Flash
	{
		...
		sr6p3_info->low_max_index = 0;
		sr6p3_info->large_max_index = 0;
		sr6p3_info->high_max_index = 4;
		sr6p3_info->mid_max_index = 0;
	}
...
static int sr6p3_auto_probe(struct flash_bank *bank)
{
	struct sr6p3_flash_bank *sr6p3_info = bank->driver_priv;
	...
	if (sr6p3_info->probed)
		return ERROR_OK;
	return sr6p3_probe(bank);
}


static int get_sr6p3_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	LOG_DEBUG("%s:%d %s()",
		__FILE__, __LINE__, __func__);
...
struct flash_driver sr6p3_flash = {
	.name = "sr6p3",
	.flash_bank_command = sr6p3_flash_bank_command,
	.erase = sr6p3_erase,
	.protect = sr6p3_protect,
	.write = sr6p3_write,
	.read = default_flash_read,
	.probe = sr6p3_probe,
	.auto_probe = sr6p3_auto_probe,
