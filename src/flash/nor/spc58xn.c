...
/* flash bank spc58xn <base> <size> 0 0 <target#> */
FLASH_BANK_COMMAND_HANDLER(spc58xn_flash_bank_command)
{
	struct spc58xn_flash_bank *spc58xn_info;
	...
	spc58xn_info = malloc(sizeof(struct spc58xn_flash_bank));
	bank->driver_priv = spc58xn_info;

	spc58xn_info->probed = 0;
	spc58xn_info->user_bank_addr = bank->base;
	spc58xn_info->user_bank_size = bank->size;

	return ERROR_OK;
}

static int spc58xn_protect_check(struct flash_bank *bank)
{
...
static int spc58xn_setlock(struct flash_bank *bank, uint32_t block_space, uint32_t lock_state)
{
	int err;
	struct target *target = bank->target;
	struct spc58xn_flash_bank *spc58xn_info = bank->driver_priv;
	struct working_area *ssd_config;
	struct working_area *setlock_algorithm;
	struct reg_param reg_params[5];
	struct powerpc_algorithm powerpc_info;
	SSD_CONFIG *ssd = &spc58xn_info->ssd;
...
static int spc58xn_getlock(struct flash_bank *bank,
...
static int spc58xn_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	if (bank->base == 0x00FC0000)
	{
		for (i = first; i <= last; i++) {
			if (i < spc58xn_info->low_max_index) {
				low_mask |= (1 << LOW_LOCK_SEL_CODE[i]);
			} else if (i < spc58xn_info->large_max_index) {
				nLargeBlockSelect.firstLargeBlockSelect |= (1 << (i - spc58xn_info->low_max_index));
			}
		}
	}
	...
	{
		for (i = first; i <= last; i++) {
			if (i < spc58xn_info->mid_max_index) {
				mid_mask |= (1 << i);
			}
		}
	}
	...
	{
		for (i = first; i <= last; i++) {
			if (i < spc58xn_info->high_max_index) {
				high_mask |= (1 << i);
			}
		}
	}
	...
	if (low_mask != 0) {
		err = spc58xn_getlock(bank, C55_BLOCK_LOW, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58xn_setlock(bank, C55_BLOCK_LOW, (lock_state & 0xFFFFF000));
		if (err != ERROR_OK)
			return err;
	}

	if (mid_mask != 0) {
		err = spc58xn_getlock(bank, C55_BLOCK_MID,  &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58xn_setlock(bank, C55_BLOCK_MID, (lock_state & 0xFFFFFFFC));
		if (err != ERROR_OK)
			return err;
	}

	if (high_mask != 0) {
		err = spc58xn_getlock(bank, C55_BLOCK_HIGH, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58xn_setlock(bank, C55_BLOCK_HIGH, (lock_state & 0xFFFFFFF0));
		if (err != ERROR_OK)
			return err;
	}

	if ((nLargeBlockSelect.firstLargeBlockSelect != 0) || (nLargeBlockSelect.secondLargeBlockSelect != 0)) {
		err = spc58xn_getlock(bank, C55_BLOCK_LARGE_FIRST, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58xn_setlock(bank, C55_BLOCK_LARGE_FIRST, (lock_state & 0xFF000000));
		if (err != ERROR_OK)
			return err;
...
static int spc58xn_protect(struct flash_bank *bank, int set, unsigned int first, unsigned int last)
{
	struct target *target = bank->target;
...
static int spc58xn_write(struct flash_bank *bank, const uint8_t *buffer,
...
static int spc58xn_probe(struct flash_bank *bank)
{
	struct spc58xn_flash_bank *spc58xn_info = bank->driver_priv;
	struct target *target = bank->target;
	struct working_area *ssd_config;
	...
	if (spc58xn_info->user_bank_size) {
		LOG_DEBUG("ignoring flash probed value, using configured bank size");
		flash_size_in_kb = spc58xn_info->user_bank_size / 1024;
	}

	LOG_INFO("flash: %d kbytes @ 0x%08x", flash_size_in_kb, spc58xn_info->user_bank_addr);
	...
	*/
	...
	if(bank->base == 0x00FC0000)
	{
		...
		bank->base = spc58xn_info->user_bank_addr;
		...
		setup_sector(bank, 10, 22, 256 * 1024);

		spc58xn_info->low_max_index = 8;
		spc58xn_info->large_max_index = 32;
		spc58xn_info->high_max_index = 0;
		spc58xn_info->mid_max_index = 0;
	}
	else if(bank->base == 0x00800000)
	{
		num_pages = 4;
		...
		spc58xn_info->low_max_index = 0;
		spc58xn_info->large_max_index = 0;
		spc58xn_info->high_max_index = 4;
		spc58xn_info->mid_max_index = 0;
	}
	else if(bank->base == 0x00404000)
	{
...
static int spc58xn_auto_probe(struct flash_bank *bank)
{
	struct spc58xn_flash_bank *spc58xn_info = bank->driver_priv;
	...
	if (spc58xn_info->probed)
		return ERROR_OK;
	return spc58xn_probe(bank);
}


static int get_spc58xn_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	LOG_DEBUG("%s:%d %s()",
		__FILE__, __LINE__, __func__);
...
struct flash_driver spc58xn_flash = {
	.name = "spc58xn",
	.flash_bank_command = spc58xn_flash_bank_command,
	.erase = spc58xn_erase,
	.protect = spc58xn_protect,
	.write = spc58xn_write,
	.read = default_flash_read,
	.probe = spc58xn_probe,
	.auto_probe = spc58xn_auto_probe,
	.erase_check = default_flash_blank_check,
	.protect_check = spc58xn_protect_check,
	.info = get_spc58xn_info,
};
