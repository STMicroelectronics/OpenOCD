...
/* flash bank spc58x <base> <size> 0 0 <target#> */
FLASH_BANK_COMMAND_HANDLER(spc58x_flash_bank_command)
{
	struct spc58x_flash_bank *spc58x_info;
	...
	spc58x_info = malloc(sizeof(struct spc58x_flash_bank));
	bank->driver_priv = spc58x_info;

	spc58x_info->probed = 0;
	spc58x_info->user_bank_addr = bank->base;
	spc58x_info->user_bank_size = bank->size;

	return ERROR_OK;
}

static int spc58x_protect_check(struct flash_bank *bank)
{
...
static int spc58x_setlock(struct flash_bank *bank, uint32_t block_space, uint32_t lock_state)
{
	int err;
	struct target *target = bank->target;
	struct spc58x_flash_bank *spc58x_info = bank->driver_priv;
...
static int spc58x_getlock(struct flash_bank *bank,
...
static int spc58x_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	if (bank->base == 0xFC0000)
	{
		for (i = first; i <= last; i++) {
			if (i < spc58x_info->low_max_index) {
				if (i>1)
					low_mask |= (1 <<  (i + 2));
				else
					low_mask |= (1 <<  (i + 1));
			} else if (i < spc58x_info->large_max_index) {
				nLargeBlockSelect.firstLargeBlockSelect |= (1 << (i - spc58x_info->low_max_index));
			} else if (i < spc58x_info->mid_max_index) {
				mid_mask |= (1 << (i - spc58x_info->low_max_index));
			} else if (i < spc58x_info->high_max_index) {
				high_mask |= (1 << (i - spc58x_info->mid_max_index));
			}
		}
	}
	...
	{
		for (i = first; i <= last; i++) {
			if (i < spc58x_info->high_max_index) {
				high_mask |= (1 << i);
			}
		}
	}
	...
	if (low_mask != 0) {
		err = spc58x_getlock(bank, C55_BLOCK_LOW, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58x_setlock(bank, C55_BLOCK_LOW, (lock_state & 0xFFFFFF00));
		if (err != ERROR_OK)
			return err;
	}

	if (mid_mask != 0) {
		err = spc58x_getlock(bank, C55_BLOCK_MID,  &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58x_setlock(bank, C55_BLOCK_MID, (lock_state & 0xFFFFFFFC));
		if (err != ERROR_OK)
			return err;
	}

	if (high_mask != 0) {
		err = spc58x_getlock(bank, C55_BLOCK_HIGH, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58x_setlock(bank, C55_BLOCK_HIGH, (lock_state & 0xFFFFFFF0));
		if (err != ERROR_OK)
			return err;
	}

	if (nLargeBlockSelect.firstLargeBlockSelect != 0) {
		err = spc58x_getlock(bank, C55_BLOCK_LARGE_FIRST, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58x_setlock(bank, C55_BLOCK_LARGE_FIRST, (lock_state & 0xFFFFFF00));
		if (err != ERROR_OK)
			return err;
...
static int spc58x_protect(struct flash_bank *bank, int set, unsigned int first, unsigned int last)
{
	struct target *target = bank->target;
...
static int spc58x_probe(struct flash_bank *bank)
{
	struct spc58x_flash_bank *spc58x_info = bank->driver_priv;
	struct target *target = bank->target;
	struct working_area *ssd_config;
	...
	if (spc58x_info->user_bank_size) {
		LOG_DEBUG("ignoring flash probed value, using configured bank size");
		flash_size_in_kb = spc58x_info->user_bank_size / 1024;
	}

	LOG_INFO("flash: %d kbytes @ 0x%08x", flash_size_in_kb, spc58x_info->user_bank_addr);
	...
	*/
	if(bank->base == 0xFC0000)
	{
		...
		bank->base = spc58x_info->user_bank_addr;
		...
		setup_sector(bank, 2, 3, 32 * 1024);

		// Large Flash Blocks
		setup_sector(bank, 5, 1, 128 * 1024);
		setup_sector(bank, 6, 7, 256 * 1024);

		spc58x_info->low_max_index = 5;
		spc58x_info->large_max_index = 13;
		spc58x_info->high_max_index = 0;
		spc58x_info->mid_max_index = 0;
	}
	else if(bank->base == 0x60C000)
	{
		num_pages = 2;
		...
		setup_sector(bank, 1, 1, 128 * 1024);

		spc58x_info->low_max_index = 2;
		spc58x_info->large_max_index = 0;
		spc58x_info->high_max_index = 0;
		spc58x_info->mid_max_index = 0;
	}
	else if(bank->base == 0x800000)
	{
		...
		spc58x_info->low_max_index = 0;
		spc58x_info->large_max_index = 0;
		spc58x_info->high_max_index = 4;
		spc58x_info->mid_max_index = 0;
	}
...
/*
	spc58x_info->low_max_index = fast_target_buffer_get_u32(&ssd->lowBlockInfo.n16KBlockNum, false) + fast_target_buffer_get_u32(&ssd->lowBlockInfo.n32KBlockNum, false) + fast_target_buffer_get_u32(&ssd->lowBlockInfo.n64KBlockNum, false) + fast_target_buffer_get_u32(&ssd->lowBlockInfo.n128KBlockNum, false);
	spc58x_info->large_max_index = spc58x_info->low_max_index +  fast_target_buffer_get_u32(&ssd->nLargeBlockNum, false);
	spc58x_info->high_max_index = spc58x_info->large_max_index + fast_target_buffer_get_u32(&ssd->highBlockInfo.n16KBlockNum, false) + fast_target_buffer_get_u32(&ssd->highBlockInfo.n32KBlockNum, false) + fast_target_buffer_get_u32(&ssd->highBlockInfo.n64KBlockNum, false) + fast_target_buffer_get_u32(&ssd->highBlockInfo.n128KBlockNum, false);
	spc58x_info->mid_max_index = spc58x_info->high_max_index + fast_target_buffer_get_u32(&ssd->midBlockInfo.n16KBlockNum, false) + fast_target_buffer_get_u32(&ssd->midBlockInfo.n32KBlockNum, false) + fast_target_buffer_get_u32(&ssd->midBlockInfo.n64KBlockNum, false) + fast_target_buffer_get_u32(&ssd->midBlockInfo.n128KBlockNum, false);
*/



	/* Done */
	spc58x_info->probed = 1;
...
static int spc58x_auto_probe(struct flash_bank *bank)
{
	struct spc58x_flash_bank *spc58x_info = bank->driver_priv;
	...
	if (spc58x_info->probed)
		return ERROR_OK;
	return spc58x_probe(bank);
}


static int get_spc58x_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	LOG_DEBUG("%s:%d %s()",
		__FILE__, __LINE__, __func__);
...
struct flash_driver spc58x_flash = {
	.name = "spc58x",
	.flash_bank_command = spc58x_flash_bank_command,
	.erase = spc58x_erase,
	.protect = spc58x_protect,
	.write = spc58x_write,
	.read = default_flash_read,
	.probe = spc58x_probe,
	.auto_probe = spc58x_auto_probe,
	.erase_check = default_flash_blank_check,
	.protect_check = spc58x_protect_check,
	.info = get_spc58x_info,
};
