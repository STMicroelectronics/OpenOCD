...
struct stmqspi_flash_bank {
	...
	int sfdp_dummy1;	/* number of dummy bytes for SFDP read for flash1 and octo */
	int sfdp_dummy2;	/* number of dummy bytes for SFDP read for flash2 */
};
...
static int read_status_reg(struct flash_bank *bank, uint16_t *status)
{
	...
	retval = target_write_u32(target, io_base + SPI_DLR,
		(stmqspi_info->saved_cr & (1U<<SPI_DUAL_FLASH)) ? 1 : 0);
	if (retval != ERROR_OK)
		goto err;
...
static int find_sfdp_dummy(struct flash_bank *bank, int len)
{
	...
	uint8_t data;
	int count, retval;
	int dual = (stmqspi_info->saved_cr & (1U<<SPI_DUAL_FLASH)) ? 1 : 0;
	bool flash1 = !(stmqspi_info->saved_cr & (1U<<SPI_FSEL_FLASH));
	const int max_bytes = 64;

	LOG_DEBUG("%s: len=%d, dual=%d, flash1=%d",
	...
	for (count = 0 ; count < max_bytes; count++) {
		...
		if (data == 0x53) {
			LOG_DEBUG("start of SFDP header for flash%c after %d dummy bytes",
				flash1 ? '1' : '2', count);
			if (flash1)
	...
	retval = ERROR_FAIL;
	LOG_DEBUG("no start of SFDP header even after %d dummy bytes", count);
...
static int read_flash_id(struct flash_bank *bank, uint32_t *id1, uint32_t *id2)
{
	...
	uint32_t io_base = stmqspi_info->io_base;
	uint8_t byte;
	int type, count, len1, len2, retval;
