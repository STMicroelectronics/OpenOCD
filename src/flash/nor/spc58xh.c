...
/* flash bank spc58xh <base> <size> 0 0 <target#> */
FLASH_BANK_COMMAND_HANDLER(spc58xh_flash_bank_command)
{
	struct spc58xh_flash_bank *spc58xh_info;
	...
	spc58xh_info = malloc(sizeof(struct spc58xh_flash_bank));
	bank->driver_priv = spc58xh_info;

	spc58xh_info->probed = 0;
	spc58xh_info->user_bank_addr = bank->base;
	spc58xh_info->user_bank_size = bank->size;

	return ERROR_OK;
}

static int spc58xh_protect_check(struct flash_bank *bank)
{
...
static int spc58xh_setlock(struct flash_bank *bank, uint32_t block_space, uint32_t lock_state)
{
	int err;
	struct target *target = bank->target;
	struct spc58xh_flash_bank *spc58xh_info = bank->driver_priv;
	struct working_area *ssd_config;
	struct working_area *setlock_algorithm;
	struct reg_param reg_params[5];
	struct powerpc_algorithm powerpc_info;
	SSD_CONFIG *ssd = &spc58xh_info->ssd;
...
static int spc58xh_getlock(struct flash_bank *bank,
...
static int spc58xh_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	if (bank->base == 0x00FC0000)
	{
		for (i = first; i <= last; i++) {
			if (i < spc58xh_info->low_max_index) {
				low_mask |= (1 << LOW_LOCK_SEL_CODE[i]);
			} else if (i < spc58xh_info->large_max_index) {
				if (i < (32 + spc58xh_info->low_max_index))
					nLargeBlockSelect.firstLargeBlockSelect |= (1 << (i - spc58xh_info->low_max_index));
				else
					nLargeBlockSelect.secondLargeBlockSelect |= (1 << (i - spc58xh_info->low_max_index - 32));
			}
		}
	}
	else if (bank->base == 0x00800000)
	{
		for (i = first; i <= last; i++) {
			if (i < spc58xh_info->high_max_index) {
				high_mask |= (1 << i);
			}
		}
	}
	else if (bank->base == 0x00404000)
		{
			for (i = first; i <= last; i++) {
				low_mask |= (1 << i);
			}
		}
	else if (bank->base == 0x00608000)
	{
		for (i = first; i <= last; i++) {
			if (i < spc58xh_info->low_max_index) {
	...
	else if (bank->base == 0x00680000)
	{
		for (i = first; i <= last; i++) {
			if (i < spc58xh_info->mid_max_index) {
				mid_mask |= (1 << i);
			}
		}
	}
	...
	if (low_mask != 0) {
		err = spc58xh_getlock(bank, C55_BLOCK_LOW, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58xh_setlock(bank, C55_BLOCK_LOW, (lock_state & 0xFFFFF000));
		if (err != ERROR_OK)
			return err;
	}

	if (mid_mask != 0) {
		err = spc58xh_getlock(bank, C55_BLOCK_MID,  &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58xh_setlock(bank, C55_BLOCK_MID, (lock_state & 0xFFFFFFFC));
		if (err != ERROR_OK)
			return err;
	}

	if (high_mask != 0) {
		err = spc58xh_getlock(bank, C55_BLOCK_HIGH, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58xh_setlock(bank, C55_BLOCK_HIGH, (lock_state & 0xFFFFFFF0));
		if (err != ERROR_OK)
			return err;
	}

	if (nLargeBlockSelect.firstLargeBlockSelect != 0) {
		err = spc58xh_getlock(bank, C55_BLOCK_LARGE_FIRST, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58xh_setlock(bank, C55_BLOCK_LARGE_FIRST, (lock_state & 0x00000000));
		if (err != ERROR_OK)
			return err;
	...
	if (nLargeBlockSelect.secondLargeBlockSelect != 0) {
		err = spc58xh_getlock(bank, C55_BLOCK_LARGE_SECOND, &lock_state);
		if (err != ERROR_OK)
			return err;

		err = spc58xh_setlock(bank, C55_BLOCK_LARGE_SECOND, (lock_state & 0xFFFFFF00));
		if (err != ERROR_OK)
			return err;
...
static int spc58xh_protect(struct flash_bank *bank, int set, unsigned int first, unsigned int last)
{
	struct target *target = bank->target;
...
static int spc58xh_write(struct flash_bank *bank, const uint8_t *buffer,
...
static int spc58xh_probe(struct flash_bank *bank)
{
	struct spc58xh_flash_bank *spc58xh_info = bank->driver_priv;
	struct target *target = bank->target;
	struct working_area *ssd_config;
	...
	if (spc58xh_info->user_bank_size) {
		LOG_DEBUG("ignoring flash probed value, using configured bank size");
		flash_size_in_kb = spc58xh_info->user_bank_size / 1024;
	}

	LOG_INFO("flash: %d kbytes @ 0x%08x", flash_size_in_kb, spc58xh_info->user_bank_addr);
	...
	*/
	...
	if(bank->base == 0x00FC0000)
	{
		...
		bank->base = spc58xh_info->user_bank_addr;
		...
		setup_sector(bank, 10, 38, 256 * 1024);

		spc58xh_info->low_max_index = 8;
		spc58xh_info->large_max_index = 48;
		spc58xh_info->high_max_index = 0;
		spc58xh_info->mid_max_index = 0;
	}
	else if(bank->base == 0x00800000)
	{
		num_pages = 4;
		...
		// Low Flash Blocks
		setup_sector(bank, 0, 4, 64 * 1024);

		spc58xh_info->low_max_index = 0;
		spc58xh_info->large_max_index = 0;
		spc58xh_info->high_max_index = 4;
		spc58xh_info->mid_max_index = 0;
	}
	else if(bank->base == 0x00404000)
	{
	...
	else if(bank->base == 0x00608000)
	{
		...
		setup_sector(bank, 1, 1, 64 * 1024);
		setup_sector(bank, 2, 1, 96 * 1024);

		spc58xh_info->low_max_index = 3;
		spc58xh_info->large_max_index = 0;
		spc58xh_info->high_max_index = 0;
		spc58xh_info->mid_max_index = 0;
	}
...
static int spc58xh_auto_probe(struct flash_bank *bank)
{
	struct spc58xh_flash_bank *spc58xh_info = bank->driver_priv;
	...
	if (spc58xh_info->probed)
		return ERROR_OK;
	return spc58xh_probe(bank);
}


static int get_spc58xh_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	LOG_DEBUG("%s:%d %s()",
		__FILE__, __LINE__, __func__);
...
struct flash_driver spc58xh_flash = {
	.name = "spc58xh",
	.flash_bank_command = spc58xh_flash_bank_command,
	.erase = spc58xh_erase,
	.protect = spc58xh_protect,
	.write = spc58xh_write,
	.read = default_flash_read,
	.probe = spc58xh_probe,
	.auto_probe = spc58xh_auto_probe,
	.erase_check = default_flash_blank_check,
	.protect_check = spc58xh_protect_check,
	.info = get_spc58xh_info,
};
