...
FLASH_BANK_COMMAND_HANDLER(sr6g6_flash_bank_command)
{
	struct sr6g6_flash_bank *sr6g6_info;
	...
	sr6g6_info = malloc(sizeof(struct sr6g6_flash_bank));
	bank->driver_priv = sr6g6_info;

	sr6g6_info->probed = 0;
	sr6g6_info->user_bank_addr = bank->base;
	sr6g6_info->user_bank_size = bank->size;

	return ERROR_OK;
}

static int sr6g6_protect_check(struct flash_bank *bank)
{
...
static int sr6g6_setlock(struct flash_bank *bank, uint32_t block_space, uint32_t lock_state)
{
	int err;
	struct target *target = bank->target;
	struct sr6g6_flash_bank *sr6g6_info = bank->driver_priv;
	struct working_area *ssd_config;
	struct working_area *setlock_algorithm;
	struct reg_param reg_params[5];
	struct armv8_algorithm armv8_algorithm_info;
	SSD_CONFIG *ssd = &sr6g6_info->ssd;
	SSD_CONFIG ssd_lock;
...
static int sr6g6_getlock(struct flash_bank *bank,
...
static int sr6g6_write(struct flash_bank *bank, const uint8_t *buffer,
		uint32_t offset, uint32_t count)
{
	...
	err = sr6g6_getlock(bank, NVM_BLOCK_LOW, &lock_state);
	if (err != ERROR_OK)
		return err;

	err = sr6g6_setlock(bank, NVM_BLOCK_LOW, (lock_state & 0xFFFF0F00));
	if (err != ERROR_OK)
		return err;

	err = sr6g6_getlock(bank, NVM_BLOCK_MID,  &lock_state);
	if (err != ERROR_OK)
		return err;

	err = sr6g6_setlock(bank, NVM_BLOCK_MID, (lock_state & 0xFFFFFF00));
	if (err != ERROR_OK)
		return err;

	err = sr6g6_getlock(bank, NVM_BLOCK_HIGH, &lock_state);
	if (err != ERROR_OK)
		return err;

	err = sr6g6_setlock(bank, NVM_BLOCK_HIGH, (lock_state & 0xFFFFFF00));
	if (err != ERROR_OK)
		return err;
	err = sr6g6_getlock(bank, NVM_BLOCK_256_FIRST, &lock_state);
	if (err != ERROR_OK)
		return err;

	err = sr6g6_setlock(bank, NVM_BLOCK_256_FIRST, (lock_state & 0xF0000000));
	if (err != ERROR_OK)
		return err;

	if (bank->base == 0x29F87000)
	{
		err = sr6g6_getlock(bank, NVM_BLOCK_UTEST,  &lock_state);
		if (err != ERROR_OK)
			return err;
		err = sr6g6_setlock(bank, NVM_BLOCK_UTEST, lock_state & 0xFFFFFFF0);
		if (err != ERROR_OK)
			return err;
	}
...
static int sr6g6_writeToErase(struct flash_bank *bank, const uint8_t *buffer,
		uint32_t offset, uint32_t count)
{
	LOG_DEBUG("%s:%d %s()", __FILE__, __LINE__, __func__);
	LOG_DEBUG("%s:%d %s() offset = 0x%08x count = 0x%08x", __FILE__, __LINE__, __func__, offset, count);
...
static int sr6g6_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	if (bank->base == 0x28000000)
	{
		for (i = first; i <= last; i++) {
			if (i < sr6g6_info->low_max_index) {
				low_mask |= (1 <<  (i + 1));
			} else if (i < sr6g6_info->large_max_index) {
				if (i == 19)
				{
					nLargeBlockSelect.first256BlockSelect |= (1 << (i - sr6g6_info->low_max_index - 1));
				}
				else
				{
					nLargeBlockSelect.first256BlockSelect |= (1 << (i - sr6g6_info->low_max_index));
				}
			} else if (i < sr6g6_info->mid_max_index) {
				mid_mask |= (1 << (i - sr6g6_info->low_max_index));
			} else if (i < sr6g6_info->high_max_index) {
				high_mask |= (1 << (i - sr6g6_info->mid_max_index));
			}
		}
	}
	...
	{
		for (i = first; i <= last; i++) {
			if (i < sr6g6_info->mid_max_index) {
				mid_mask |= (1 << i);
			} else if (i < sr6g6_info->large_max_index) {
				nLargeBlockSelect.first256BlockSelect |= (1 << ((i - sr6g6_info->mid_max_index)+14));
			} else if (i < sr6g6_info->low_max_index) {
				low_mask |= (1 << i);
			} else if (i < sr6g6_info->high_max_index) {
				high_mask |= (1 << i);
			}
	...
	else if (bank->base == 0x29400000)
	{
		for (i = first; i <= last; i++) {
			if (i < sr6g6_info->mid_max_index) {
				...
				nLargeBlockSelect.first256BlockSelect |= (1 << ((i - sr6g6_info->mid_max_index) + 7));
			} else if (i < sr6g6_info->low_max_index) {
				low_mask |= (1 << i);
			} else if (i < sr6g6_info->high_max_index) {
				high_mask |= (1 << i);
	...
	{
		for (i = first; i <= last; i++) {
			if (i < sr6g6_info->high_max_index) {
				high_mask |= (1 << i);
			} else if (i < sr6g6_info->large_max_index) {
				nLargeBlockSelect.first256BlockSelect |= (1 << ((i - sr6g6_info->mid_max_index) + 7));
			} else if (i < sr6g6_info->low_max_index) {
...
*/
	...
	for (i=first; i<= last; i++)
	{
		sr6g6_writeToErase(bank, buffer, bank->sectors[i].offset, bank->sectors[i].size);
...
static int sr6g6_protect(struct flash_bank *bank, int set, unsigned int first, unsigned int last)
{
	struct target *target = bank->target;
...
static int sr6g6_probe(struct flash_bank *bank)
{
	struct sr6g6_flash_bank *sr6g6_info = bank->driver_priv;
	struct target *target = bank->target;
	struct working_area *ssd_config;
	...
	if (sr6g6_info->user_bank_size) {
		LOG_DEBUG("ignoring flash probed value, using configured bank size");
		flash_size_in_kb = sr6g6_info->user_bank_size / 1024;
	}

	LOG_INFO("flash: %d kbytes @ 0x%08x", flash_size_in_kb, sr6g6_info->user_bank_addr);
	...
	*/
	...
	if(bank->base == 0x28000000)   //Cluster 0 core 0
	{
		...
		bank->base = sr6g6_info->user_bank_addr;
		...
		// Large Flash Blocks
		setup_sector(bank, 8, 10, 256 * 1024);
		setup_sector(bank, 18, 2, 128 * 1024);
		setup_sector(bank, 20, 3, 256 * 1024);


		sr6g6_info->low_max_index = 8;
		sr6g6_info->large_max_index = 23;
		sr6g6_info->high_max_index = 0;
		sr6g6_info->mid_max_index = 0;
	}
	if(bank->base == 0x28400000)   //Cluster 0 core 1
	{
		...
		sr6g6_info->low_max_index = 0;
		sr6g6_info->large_max_index = 23;
		sr6g6_info->high_max_index = 0;
		sr6g6_info->mid_max_index = 8;
	}
	...
	else if(bank->base == 0x29E00000) //Cluster 0 Core 0 Data Flash
	{
		...
		sr6g6_info->low_max_index = 0;
		sr6g6_info->large_max_index = 0;
		sr6g6_info->high_max_index = 4;
		sr6g6_info->mid_max_index = 0;
	}
...
static int sr6g6_auto_probe(struct flash_bank *bank)
{
	struct sr6g6_flash_bank *sr6g6_info = bank->driver_priv;
	...
	if (sr6g6_info->probed)
		return ERROR_OK;
	return sr6g6_probe(bank);
}


static int get_sr6g6_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	LOG_DEBUG("%s:%d %s()",
		__FILE__, __LINE__, __func__);
...
struct flash_driver sr6g6_flash = {
	.name = "sr6g6",
	.flash_bank_command = sr6g6_flash_bank_command,
	.erase = sr6g6_erase,
	.protect = sr6g6_protect,
	.write = sr6g6_write,
	.read = default_flash_read,
	.probe = sr6g6_probe,
	.auto_probe = sr6g6_auto_probe,
	.erase_check = default_flash_blank_check,
	.protect_check = sr6g6_protect_check,
	.info = get_sr6g6_info,
