...
#define FLASH_ERROR (FLASH_PROGERR | FLASH_PGSERR | FLASH_SIZERR | FLASH_PGAERR \
			| FLASH_WRPERR | FLASH_OPERR | FLASH_OPTWERR)
...
static int stm32x_probe(struct flash_bank *bank)
{
	...
	LOG_INFO("STM32L5xx flash size is %dkb, base address is 0x%" PRIx32, flash_size_in_kb, bank->base);
...
{
	if (CMD_ARGC < 1) {
		command_print(CMD_CTX, "stm32l5x mass_erase <bank>");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
	...
	if (retval == ERROR_OK) {
		...
		command_print(CMD_CTX, "stm32l5x mass erase complete");
	} else {
		command_print(CMD_CTX, "stm32l5x mass erase failed");
	}
...
{
	if (CMD_ARGC < 2) {
		command_print(CMD_CTX, "stm32l5x window_watchdog_soft_selection bank_id [enable|disable]");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
...
{
	if (CMD_ARGC < 2) {
		command_print(CMD_CTX, "stm32l5x independent_watchdog_standby bank_id [enable|disable]");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
...
{
	if (CMD_ARGC < 2) {
		command_print(CMD_CTX, "stm32l5x independent_watchdog_stop bank_id [enable|disable]");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
...
{
	if (CMD_ARGC < 2) {
		command_print(CMD_CTX, "stm32l5x independent_watchdog_soft_selection bank_id [enable|disable]");
		return ERROR_COMMAND_SYNTAX_ERROR;
	}
