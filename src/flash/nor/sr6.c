...
static int sr6_setlock(struct flash_bank *bank, uint32_t block_space, uint32_t lock_state)
{
	...
	if((bank->base == 0x28000000)||(bank->base == 0x28400000)||(bank->base == 0x29E00000))   //Cluster 0 core 0 || Cluster 0 core 1
	{
...
static int sr6_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	if (low_mask != 0) {
		...
		err = sr6_setlock(bank, NVM_BLOCK_LOW, (lock_state & 0xFFFFFF00));
		if (err != ERROR_OK)
			return err;
	}
