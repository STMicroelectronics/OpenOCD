...
struct powerpc56_core_reg {
	uint32_t num;
	struct target *target;
	struct powerpc56_common *powerpc56_common;
};

static inline
struct powerpc56_common *target_to_powerpc(struct target *target)
{
	return target->arch_info;
}

extern const struct command_registration powerpc56_command_handlers[];
/*
static int powerpc56_write_memory_noturn(struct target *target, target_addr_t address,
