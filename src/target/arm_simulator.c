	...
	uint32_t *dry_run_pc, struct arm_sim_interface *sim)
{
	...
	if (sim->get_state(sim) == ARM_STATE_ARM) {
		...
		if ((opcode & 0xf800) == 0xf000) {
			uint32_t high = instruction.info.b_bl_bx_blx_bxns_blxns.target_address;
			...
			if (retval != ERROR_OK)
				return retval;
			instruction.info.b_bl_bx_blx_bxns_blxns.target_address += high;
		}
	}
	...
	if ((instruction.type >= ARM_B) && (instruction.type <= ARM_BLX)) {
		...
		if (instruction.info.b_bl_bx_blx_bxns_blxns.reg_operand == -1)
			target_address = instruction.info.b_bl_bx_blx_bxns_blxns.target_address;
		else {
			target_address = sim->get_reg_mode(sim,
					instruction.info.b_bl_bx_blx_bxns_blxns.reg_operand);
			if (instruction.info.b_bl_bx_blx_bxns_blxns.reg_operand == 15)
