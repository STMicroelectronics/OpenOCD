...
static int powerpc56_save_context(struct target *target);
static int powerpc56_restore_context(struct target *target);
static int powerpc56_set_watchpoint(struct target *target, struct watchpoint *watchpoint);
static int powerpc56_set_breakpoint(struct target *target, struct breakpoint *breakpoint);
static int powerpc56_once_spr_read(struct target *target, uint32_t reg_id, uint32_t *val);
static int powerpc56_once_spr_write(struct target *target, uint32_t reg_id, uint32_t val);
static int powerpc56_assert_reset(struct target *target);
static int powerpc56_unset_breakpoint(struct target *target, struct breakpoint *breakpoint);
static void powerpc56_once_get_idcode(struct target *target);
//static int powerpc56_nexus_mem_read(struct target *target, target_addr_t address,	uint32_t size, uint32_t count, uint32_t *buffer);
static int powerpc56_once_reg_write(struct jtag_tap *tap, int flush, uint8_t reg, uint32_t * data);
static int powerpc56_once_target_status(struct jtag_tap *tap);
...
/* Bolero ? */
#define JTAG_INSTR_ACCESS_AUX_TAP_TCU   0x1B
...
#define  POWERPC_TLBWE_OPCODE  (0x7C0007A4)
...
#define EXTRACT_PRN(X)        (((X) & 0xF0000000) >> 28)
#define EXTRACT_DESIGN(X)     (((X) & 0x0FC00000) >> 22)
#define EXTRACT_PIN(X)        (((X) & 0x003FF000) >> 12)
#define EXTRACT_VER_MAJOR(X)  (((X) & 0x0f000000) >> 30)
#define EXTRACT_VER_MINOR(X)  (((X) & 0xf0000000) >> 28)

#define JEP106_STMICROELECTRONICS  0x020

static void powerpc56_decode_idcode(uint32_t idcode)
{
	...
	if (manufacturer == JEP106_STMICROELECTRONICS) {
		...
		uint32_t part_number = EXTRACT_PIN(idcode);
		uint32_t rev_major = EXTRACT_VER_MAJOR(idcode) + 1;
		uint32_t rev_minor = EXTRACT_VER_MINOR(idcode) - 1;

		char *device_name = NULL;


		if (part_number == 0x241)
		{
			device_name="SPC560C40/50";
		}
		else if (part_number == 0x243)
		{
			device_name="SPC560B40/50";
		}
		else if (part_number == 0x244)
		{
			device_name="SPC560D30/40";
		}
		else if (part_number == 0x249)
		{
			device_name="SPC564B/SPC56EC";
		}
		else
		{
			device_name="unknown";
		...
		LOG_INFO("0x%08x "
				...
				(unsigned int)EXTRACT_PIN(idcode),
				(unsigned int)EXTRACT_PRN(idcode));
	}
}
...
static int powerpc56_verify_pointer(struct command_invocation *cmd, struct powerpc56_common *powerpc)
{
 ...
 */
static int powerpc56_jtag_ir(struct jtag_tap *tap, uint8_t * ir_in, uint8_t * ir_out, int ir_len)
{
	jtag_add_plain_ir_scan(ir_len, ir_out, ir_in, TAP_IDLE);
	return ERROR_OK;
}

static int powerpc56_jtag_ir_u8(struct jtag_tap *tap, uint8_t * ir_in, uint8_t ir_out, int ir_len)
{
	return powerpc56_jtag_ir(tap, ir_in, &ir_out, ir_len);
}

static int powerpc56_jtag_ir_u32(struct jtag_tap *tap, uint32_t * ir_in, uint32_t ir_out, int ir_len)
{
	return powerpc56_jtag_ir(tap, (uint8_t *) ir_in, (uint8_t *) &ir_out, ir_len);
}
 ...
 */
static
int powerpc56_jtag_send_ir(struct jtag_tap *tap, uint8_t cmd_out)
{
	return powerpc56_jtag_ir_u8(tap, NULL, cmd_out, tap->ir_length);
}

/*
 * OnCE single word instruction
 */
static int powerpc56_once_ir_exec(struct jtag_tap *tap, int flush, uint32_t instr, uint32_t rw, uint32_t go, uint32_t ex)
{
	int err;
	uint32_t ir_out;
	uint32_t ir_in;
	...
	err = powerpc56_jtag_ir_u32(tap, &ir_in, ir_out, 10);
	if (err != ERROR_OK)
		return err;
 ...
 */
static int powerpc56_once_gain_access(struct jtag_tap *tap)
{
	/* Load opcode into the JTAGC Instruction Register */
	return powerpc56_jtag_send_ir(tap, JTAG_INSTR_ACCESS_AUX_TAP_ONCE);
}

/*
 * Release OnCE TAP controller
 */
static int powerpc56_once_release(struct target *target)
{
	int err;
	uint32_t dr_in = 0;

	err = powerpc56_once_gain_access(target->tap);
	if (err != ERROR_OK)
		return err;

	err = powerpc56_once_ir_exec(target->tap, 1, E200Zxx_ONCE_NONE, 0, 0, 0);
	if (err != ERROR_OK)
		return err;
 ...
 */ 
static uint32_t powerpc56_jtag_get_idcode(struct target *target)
{
	int dr_in, dr_out, err;

	/* Make sure to leave OnCE ownership to access the main JTAGC controller */
   err = powerpc56_once_release(target);
   if (err != ERROR_OK)
      return err;

	/* Get the JTAGC IDCODE */
	dr_in = dr_out = 0;
	powerpc56_jtag_send_ir(target->tap, JTAG_INSTR_IDCODE);
	...
	if (target->state == TARGET_HALTED)
		powerpc56_once_gain_access(target->tap);

	LOG_DEBUG("JTAG IDCODE : 0x%" PRIx32, dr_in);
	return dr_in;
}

static int powerpc56_once_read_chain_regs(struct jtag_tap *tap, uint32_t * data, int len)
{
	int err;
	int i;
	uint32_t dr_out;
	...
	for (i = 0; i < len; i++) {
		...
		err = powerpc56_once_gain_access(tap);
		if (err != ERROR_OK)
			return err;

		err = powerpc56_once_ir_exec(tap, 1, E200Zxx_ONCE_CPUSCR, 1 /*rw*/, 0 /*go*/, 0 /*ex*/);
		if (err != ERROR_OK)
			return err;
...
static int powerpc56_once_write_chain_regs(struct jtag_tap *tap, uint32_t * data, int len)
{
	int err;
	int i;
	...
	for (i = 0; i < len; i++) {
		...
		err = powerpc56_once_ir_exec(tap, 1, E200Zxx_ONCE_CPUSCR, 0 /*rw*/, 0 /*go*/, 0 /*ex*/);
		if (err != ERROR_OK)
			return err;
...
#if 0
static int powerpc56_nexus_mem_write(struct target *target, target_addr_t address,
		uint32_t size, uint32_t count, const uint8_t *buffer)
{
	...
	uint32_t data_in;
		...
		err = powerpc56_once_ir_exec(target->tap, 1, E200Zxx_ONCE_NEXUS3, 0 /*rw*/, 0 /*go*/, 0 /*ex*/);
		if (err != ERROR_OK)
			return err;
...
static int powerpc56_nexus_mem_read(struct target *target, target_addr_t address,
		...
		uint32_t size, uint32_t count, uint32_t *buffer)
{
	...
	data_out = (0x1<<31 | 0x1<<30 | loc_size << 27 | 0x3 << 22 | (count * loc_count) << 2);
	jtag_add_plain_dr_scan(32, (uint8_t *) &data_out, (uint8_t *) NULL, TAP_IDLE);
	err = jtag_execute_queue();
	if (err != ERROR_OK)
		return err;
...
/** OnCE read register */
static int powerpc56_once_reg_read(struct jtag_tap *tap, int flush, uint8_t reg, uint32_t * data)
{
	int err;
	uint32_t dr_in = 0;
	...
	err = powerpc56_once_ir_exec(tap, flush, reg, 1 /*rw*/, 0 /*go*/, 0 /*ex*/);
	if (err != ERROR_OK)
		return err;
...
static void powerpc56_once_get_idcode(struct target *target)
{
		...
		powerpc56_once_reg_read(target->tap, 1, E200Zxx_ONCE_JTAG_ID, &dr_out);
...
static int powerpc56_once_reg_write(struct jtag_tap *tap, int flush, uint8_t reg, uint32_t * data)
{
	...
	/* Execute the command */
	err = powerpc56_once_ir_exec(tap, flush, reg, 0 /*rw*/, 0 /*go*/, 0 /*ex*/);
	if (err != ERROR_OK)
		return err;
...
/** OnCE write register */
static int powerpc56_once_enable(struct jtag_tap *tap)
{
	int err;
	...
	/* Execute the command */
	err = powerpc56_once_ir_exec(tap, 1, E200Zxx_ONCE_ENABLE, 0 /*rw*/, 0 /*go*/, 0 /*ex*/);
	if (err != ERROR_OK)
		return err;

	/* Execute the command */
	err = powerpc56_once_ir_exec(tap, 1, E200Zxx_ONCE_BYPASS, 1 /*rw*/, 1 /*go*/, 1 /*ex*/);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_once_read_status(struct jtag_tap *tap, uint32_t *data)
{
	int err;
	uint32_t ir_out;
	uint32_t ir_in;
...
static int powerpc56_once_read_cpu_scan_regs(struct jtag_tap *tap, struct cpu_status *cpu_info)
{
	int err;
	uint32_t data[6];

	err = powerpc56_once_read_chain_regs(tap, data, 6);
	if (err == ERROR_OK) {
...
static int powerpc56_once_write_cpu_scan_regs(struct jtag_tap *tap, struct cpu_status *cpu_info)
{
	uint32_t data[6];
	...
	return powerpc56_once_write_chain_regs(tap, data, 6);
}

#if 0
static int powerpc56_once_print_cpu_scan_regs(struct target *target)
{
	struct powerpc56_common *powerpc = target_to_powerpc(target);
 ...
 *  3) IR register
 */
static int powerpc56_once_reset_cpu_scan_regs(struct target *target)
{
	uint32_t ctl_pcofst;
...
static int powerpc56_once_exec(struct jtag_tap *tap, uint32_t go, uint32_t ex)
{
	int err;
	uint32_t dr_in = 0;
	...
	err = powerpc56_once_ir_exec(tap, 1, E200Zxx_ONCE_NONE, 0 /*rw*/, go, ex);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_once_gpr_read(struct target *target, uint32_t reg_id, uint32_t *val)
{
	int err;
	...
	err = powerpc56_once_write_cpu_scan_regs(target->tap, &cpu_info);
	if (err != ERROR_OK)
		return err;

	/* Go */
	err = powerpc56_once_exec(target->tap, 1 /*go*/, 0 /*ex*/);
	if (err != ERROR_OK)
		return err;

	/* Read CPU info */
	err = powerpc56_once_read_cpu_scan_regs(target->tap, &cpu_info);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_once_gpr_write(struct target *target, uint32_t reg_id, uint32_t val)
{
	int err;
	struct cpu_status cpu_info;
...
static int powerpc56_once_spr_read(struct target *target, uint32_t reg_id, uint32_t *val)
{
	...
	err = powerpc56_once_gpr_read(target, GPR_SCRATCH, &value1);
	if (err != ERROR_OK)
		return err;
	...
	*val = cpu_info.WBBR_LOW;
	...
	/* Restore GPR31 */
	err = powerpc56_once_gpr_write(target, GPR_SCRATCH, value1);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_once_tlbwe(struct target *target)
{
	int err;
	struct cpu_status cpu_info;
    ...
    /* mtspr reg_id, WBBR.low */
	cpu_info.IR = POWERPC_TLBWE_OPCODE;
	cpu_info.CTL = (0x80000000 >> 30) | (0x80000000 >> 21); /* Set also FFRA */
	cpu_info.WBBR_LOW = 0;
	cpu_info.PC = 0xF7FB8000;
...
static int powerpc56_once_cr_read(struct target *target, uint32_t *val)
{
	int err;
	struct cpu_status cpu_info;
...
static int powerpc56_once_cr_write(struct target *target, uint32_t val)
{
	int err;
	struct cpu_status cpu_info;
...
static int powerpc56_once_mem_read(struct target *target, uint32_t addr, uint32_t size, uint32_t *val)
{
	int err;
	struct cpu_status cpu_info;
	...
	err = powerpc56_once_gpr_read(target, GPR_SCRATCH, &gpr_scratch_value);
	if (err != ERROR_OK)
		return err;
	...
	*val = cpu_info.WBBR_LOW;
	...
	err = powerpc56_once_gpr_write(target, GPR_SCRATCH, gpr_scratch_value);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_once_mem_write(struct target *target, uint32_t addr, uint32_t size, uint32_t val, uint32_t is_vle, struct cpu_status cpu_info)
{
	int err;
	uint32_t opcode;

	/* Write value to scratch register */
	err = powerpc56_once_gpr_write(target, GPR_SCRATCH, val);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_once_target_status(struct jtag_tap *tap)
{
	...
	err = powerpc56_once_read_status(tap, &status_reg);
	if (err != ERROR_OK)
 ...
 */
static int powerpc56_once_debug_enter(struct target *target)
{
	int err;
	uint32_t value;
	...
	value = 0x00000007;
	err = powerpc56_once_reg_write(target->tap, 1, E200Zxx_ONCE_OCR, &value);
	if (err != ERROR_OK)
		return err;

	err = powerpc56_once_enable(target->tap);
	if (err != ERROR_OK)
		return err;
	...
	for (int i=0; i<100; i++) {
		 ...
		 */
		if (powerpc56_once_target_status(target->tap) == TARGET_RESET) {
	...
	/*Clear DBSR */
	value = 0xFFFFFFFF;
	err = powerpc56_once_reg_write(target->tap, 1, E200Zxx_ONCE_EDBSR0, &value);
	if (err != ERROR_OK)
		return err;


	/* Save CPU context */
	err = powerpc56_save_context(target);
	if (err != ERROR_OK)
		return err;

	/* Get and print the TAP IDCODE discovered. */
	powerpc56_once_get_idcode(target);

	return ERROR_OK;
}

static int powerpc56_write_debug_info_bits(struct target *target,
...
static int powerpc56_once_debug_exit(struct target *target)
{
	int err;
	uint32_t value;
	struct powerpc56_common *powerpc = target_to_powerpc(target);
	...
	/* Read the status register */
	err = powerpc56_once_read_status(target->tap, &status_reg);
	...
	if (powerpc->ctx_info & DEBUG_SOFT_BREAK) {
		...
		powerpc56_write_debug_info_bits(target, DEBUG_RUNNING, 0);
	} else if (powerpc->ctx_info & DEBUG_RUNNING) {
		powerpc->cpu_info.PC = pc_value;
		powerpc56_once_reset_cpu_scan_regs(target);
	} else {
		powerpc->cpu_info.PC = pc_value - 4;
		powerpc->cpu_info.IR = BUILD_INSTR(POWERPC_E_ORI_OPCODE, 0, 0, 0xD000);
		powerpc->cpu_info.CTL = (0x80000000 >> 30);
	...
	err = powerpc56_once_write_cpu_scan_regs(target->tap, &powerpc->cpu_info);
	if (err != ERROR_OK)
		return err;

	/* Go! */
	err = powerpc56_once_exec(target->tap, 1 /*go*/, 1 /*ex*/);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_handle_debug_event(struct target *target)
{
	uint32_t value;
	uint32_t dbg_status_reg;

	/* Save CPU context */
	int err = powerpc56_save_context(target);
	if (err != ERROR_OK)
		return err;

	dbg_status_reg = 0;
	err = powerpc56_once_reg_read(target->tap, 1, E200Zxx_ONCE_DBSR, &dbg_status_reg);
	if (err != ERROR_OK)
		return err;

	/* Clear status register */
	value = 0xFFFFFFFF;
	err = powerpc56_once_reg_write(target->tap, 1, E200Zxx_ONCE_DBSR, &value);
	if (err != ERROR_OK)
		return err;
	...
	if (dbg_status_reg & 0x00FF0000) {
		...
		if (breakpoint && (breakpoint->type == BKPT_SOFT)) {
			...
			pc_reg->dirty = true;
			pc_reg->valid = true;
			powerpc56_write_debug_info_bits(target, DEBUG_SOFT_BREAK, 0);
...
static const struct jim_nvp powerpc56_target_state[] = {
...
static int powerpc56_poll(struct target *target)
{
	int err;
	...
	current_target_state = powerpc56_once_target_status(target->tap);

	if (current_target_state != previous_target_state) {
		const char *previous_state;
		const char *current_state;

		previous_state = jim_nvp_value2name_simple(powerpc56_target_state, previous_target_state)->name;
		if (!previous_state)
			previous_state = "undefined (BUG ?)";

		current_state = jim_nvp_value2name_simple(powerpc56_target_state, current_target_state)->name;
		if (!current_state)
	...
	if (((current_target_state  == TARGET_HALTED)  && (previous_target_state == TARGET_UNKNOWN)) ||
		...
		/* Do we have a request to halt ? (see powerpc56_target_create) */
		if (powerpc->ctx_info & DEBUG_HALT) {

			/* We decided to stay always in debug mode */
			err = powerpc56_once_debug_enter(target);
			if (err != ERROR_OK) {
				LOG_INFO("ERROR");
				return err;
			}
			...
			powerpc56_write_debug_info_bits(target, 0, DEBUG_HALT); // set debug_halt off?
		...
		if (powerpc->ctx_info & DEBUG_HALT) {
			...
			powerpc56_write_debug_info_bits(target, 0, DEBUG_HALT);
		...
		if (powerpc->ctx_info & DEBUG_STEP) {
			...
			powerpc56_write_debug_info_bits(target, 0, DEBUG_STEP);
		} else if (previous_target_state == TARGET_DEBUG_RUNNING) {
			powerpc56_write_debug_info_bits(target, DEBUG_HALT, DEBUG_RUNNING);
		} else {
			if (previous_target_state == TARGET_RUNNING) {
				/* Breakpoint or watchpoint hit */
				err = powerpc56_handle_debug_event(target);
				if (err != ERROR_OK)
					return err;
...
static int powerpc56_halt(struct target *target)
{
	enum target_state previous_target_state = target->state;
	...
	powerpc56_once_debug_enter(target);
...
static int powerpc56_soft_reset_halt(struct target *target)
{
...
#if 0
static void powerpc56_enable_breakpoints(struct target *target)
{
	...
	while (breakpoint) {
		if (!breakpoint->is_set)
			powerpc56_set_breakpoint(target, breakpoint);
		breakpoint = breakpoint->next;
	}
}

static void powerpc56_enable_watchpoints(struct target *target)
{
	...
	while (watchpoint) {
		if (!watchpoint->is_set)
			powerpc56_set_watchpoint(target, watchpoint);
		watchpoint = watchpoint->next;
	}
}
#endif

static int powerpc56_resume(struct target *target, int current,
	target_addr_t address, int handle_breakpoints, int debug_execution)
{
	...
	if (handle_breakpoints) {
		...
		if (breakpoint)
			powerpc56_unset_breakpoint(target, breakpoint);
	}

	/* Restore CPU Context */
	err = powerpc56_restore_context(target);
	if (err != ERROR_OK)
		return err;

	/* Exit debug mode */
	err = powerpc56_once_debug_exit(target);
	if (err != ERROR_OK)
		return err;
	...
	powerpc56_write_debug_info_bits(target, DEBUG_RUNNING, DEBUG_HALT);
...
static int powerpc56_unset_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	if (breakpoint->type == BKPT_HARD) {
		...
		err = powerpc56_once_reg_read(target->tap, 1, E200Zxx_ONCE_DBCR0, &value);
		if (err != ERROR_OK)
			return err;
...
static int powerpc56_set_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	if (breakpoint->type == BKPT_HARD) {
		...
		if (iac_reg < 4) {
			err = powerpc56_once_reg_write(target->tap, 1, (iac_reg + E200Zxx_ONCE_IAC1), &value);
			if (err != ERROR_OK)
				return err;
		} else {
			err = powerpc56_once_reg_write(target->tap, 1, (iac_reg - 4 + E200Zxx_ONCE_IAC5), &value);
			if (err != ERROR_OK)
				return err;
		}
...
static int powerpc56_step(struct target *target, int current,
	target_addr_t address, int handle_breakpoints)
{
	...
	if (powerpc->ctx_info & DEBUG_SOFT_BREAK) {
		...
		cpu_info.CTL = (0x80000000 >> 30);
		powerpc56_write_debug_info_bits(target, 0, DEBUG_SOFT_BREAK);
		powerpc56_write_debug_info_bits(target, DEBUG_STEP_TWICE, 0);
	} else if (powerpc->ctx_info & DEBUG_RUNNING) {
		/* Normal running */
...
do_again:
	...
	if (powerpc->ctx_info & DEBUG_STEP_TWICE) {
		powerpc56_write_debug_info_bits(target, 0, DEBUG_STEP_TWICE);
	...
	/* Write to Debug Halting Control and Status Register */
	powerpc56_write_debug_info_bits(target, DEBUG_STEP, 0);
...
static int powerpc56_assert_reset(struct target *target)
{
	...
	//powerpc56_jtag_send_ir(target->tap, JTAG_INSTR_EXTEST);
	if (target->reset_halt) {
		adapter_assert_reset();
		powerpc56_halt(target);
	}

	return ERROR_OK;
}

static int powerpc56_deassert_reset(struct target *target)
{
	LOG_DEBUG("(%s) Entering: %s",  __func__, target_state_name(target));
	LOG_DEBUG("target->state: %s",  target_state_name(target));
...
static int powerpc56_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	return powerpc56_set_breakpoint(target, breakpoint);
}

static int powerpc56_remove_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	if (target->state != TARGET_HALTED) {
		LOG_WARNING("target not halted");
		return ERROR_TARGET_NOT_HALTED;
...
static int powerpc56_set_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	...
	err = powerpc56_once_reg_write(target->tap, 1, (dac_reg + E200Zxx_ONCE_DAC1), &value);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_add_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	if (target->state != TARGET_HALTED) {
		LOG_WARNING("target not halted");
		return ERROR_TARGET_NOT_HALTED;
	...
	return powerpc56_set_watchpoint(target, watchpoint);
}

static int powerpc56_unset_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	int err;
	uint32_t dac_reg;
	uint32_t value;
...
static int powerpc56_remove_watchpoint(struct target *target, struct watchpoint *watchpoint)
{
	if (target->state != TARGET_HALTED) {
		LOG_WARNING("target not halted");
		return ERROR_TARGET_NOT_HALTED;
	...
	return powerpc56_unset_watchpoint(target, watchpoint);
}

#if 0
static int powerpc56_read_memory(struct target *target, target_addr_t address,
	uint32_t size, uint32_t count, uint8_t *buffer)
{
	...
	/* Read memory ... */
	err = powerpc56_nexus_mem_read(target, address, size, count, (uint32_t *)(void *)buffer);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_write_memory(struct target *target, target_addr_t address,
	uint32_t size, uint32_t count, const uint8_t *buffer)
{
	int err;
	...
	/* Write memory ... */
	err = powerpc56_nexus_mem_write(target, address, size, count, (uint32_t *)(void *)buffer);
	if (err != ERROR_OK)
		return err;
	return ERROR_OK;
}
	...
	uint32_t size, uint32_t count, uint8_t *buffer)
{
	...
	if ((part_number == 0x241) || (part_number == 0x249))
	{
		/* Sanitize arguments */
		if (((size != 8) && (size != 4) && (size != 2) && (size != 1)) || (count == 0) || !(buffer))
			return ERROR_COMMAND_SYNTAX_ERROR;
	...
	else
	{
		...
		for (i = 0; i < count; i++) {
			...
			err = powerpc56_once_mem_read(target, address, size, &val);
			if (err != ERROR_OK)
				return err;
	...
	uint32_t size, uint32_t count, const uint8_t *buffer)
{
	...
	else
	{
		...
		if (((size == 8) && (address & 0x3u)) || ((size == 4) && (address & 0x3u)) || ((size == 2) && (address & 0x1u)))
			return ERROR_TARGET_UNALIGNED_ACCESS;
	...
	/*
		if ((count * size) < 8)
		{
			err = powerpc56_nexus_mem_write(target, address, size, count, (uint8_t *) &buffer);
			if (err != ERROR_OK)
				return err;
		}
		...
		for (i = 0; i < count; i++)
		{
		    for (j=0; j < loop; j++)
		    {
				...
				*d-- = *s++;
				...
				err = powerpc56_once_mem_write(target, address, loc_size, val, is_vle, cpu_info);
				if (err != ERROR_OK)
					return err;
...
static int powerpc56_save_context(struct target *target)
{
	...
	err = powerpc56_once_read_cpu_scan_regs(target->tap, &powerpc->cpu_info);
	if (err != ERROR_OK)
		return err;

	/* Read core registers */
	for (i = 0; i < POWERPC_NUM_GP_REGS; i++) {
		err = powerpc56_once_gpr_read(target, i, &powerpc->core_regs[i]);
		if (err != ERROR_OK)
			return err;
	}
	...
	err = powerpc56_once_cr_read(target, &powerpc->core_regs[CR]);
	if (err != ERROR_OK)
		return err;

	err = powerpc56_once_spr_read(target, SPR_LR, &powerpc->core_regs[LR]);
	if (err != ERROR_OK)
		return err;

	err = powerpc56_once_spr_read(target, SPR_CTR, &powerpc->core_regs[CTR]);
	if (err != ERROR_OK)
		return err;

	err = powerpc56_once_spr_read(target, SPR_XER, &powerpc->core_regs[XER]);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_restore_context(struct target *target)
{
	...
	for (i = 0; i < POWERPC_NUM_GP_REGS; i++) {
		...
		err = powerpc56_once_gpr_write(target, i, powerpc->core_regs[i]);
		if (err != ERROR_OK)
			return err;
	}
	...
	err = powerpc56_once_spr_write(target, SPR_XER, powerpc->core_regs[XER]);
	if (err != ERROR_OK)
		return err;
	...
	err = powerpc56_once_spr_write(target, SPR_LR, powerpc->core_regs[LR]);
	if (err != ERROR_OK)
		return err;
	...
	err = powerpc56_once_spr_write(target, SPR_CTR, powerpc->core_regs[CTR]);
	if (err != ERROR_OK)
		return err;
	...
	err = powerpc56_once_cr_write(target, powerpc->core_regs[CR]);
	if (err != ERROR_OK)
		return err;
...
static int powerpc56_read_core_reg(struct target *target, unsigned int num)
{
	uint32_t reg_value;
	struct powerpc56_common *powerpc = target_to_powerpc(target);
...
static int powerpc56_write_core_reg(struct target *target, unsigned int num)
{
	uint32_t reg_value;
	struct powerpc56_common *powerpc = target_to_powerpc(target);
...
static int powerpc56_get_gdb_reg_list(struct target *target, struct reg **reg_list[],
...
static int powerpc56_get_core_reg(struct reg *reg)
{
	struct powerpc56_core_reg *powerpc56_reg = reg->arch_info;
	struct target *target = powerpc56_reg->target;
	struct powerpc56_common *powerpc56_target = target_to_powerpc(target);

	if (target->state != TARGET_HALTED)
		return ERROR_TARGET_NOT_HALTED;

	return powerpc56_target->read_core_reg(target, powerpc56_reg->num);
}

static int powerpc56_set_core_reg(struct reg *reg, uint8_t *buf)
{
	struct powerpc56_core_reg *powerpc56_reg = reg->arch_info;
	struct target *target = powerpc56_reg->target;
...
int powerpc56_start_algorithm(struct target *target,
...
{
	struct powerpc56_common *powerpc = target_to_powerpc(target);
	struct powerpc56_algorithm *powerpc56_algorithm_info = arch_info;
	int retval = ERROR_OK;

	LOG_DEBUG("%s:%d %s()", __FILE__, __LINE__, __func__);

	if (powerpc56_algorithm_info->common_magic != POWERPC_COMMON_MAGIC) {
	...
	for (unsigned i = 0; i < powerpc->core_cache->num_regs; i++) {
		powerpc56_algorithm_info->context[i] = buf_get_u32(
	...
	for (int i = 0; i < num_reg_params; i++) {
		...
		powerpc56_set_core_reg(reg, reg_params[i].value);
	}

	/*
	 * NOTE: powerpc56_run_algorithm requires that each algorithm uses
...
int powerpc56_wait_algorithm(struct target *target,
	int num_mem_params, struct mem_param *mem_params,
	int num_reg_params, struct reg_param *reg_params,
...
*/
	...
	for (i = powerpc->core_cache->num_regs - 1; i >= 0; i--) {
		...
		if (regvalue != powerpc56_algorithm_info->context[i]) {

			LOG_DEBUG("restoring register %s with value 0x%8.8" PRIx32,
				powerpc->core_cache->reg_list[i].name,
				powerpc56_algorithm_info->context[i]);

			buf_set_u32(powerpc->core_cache->reg_list[i].value,
				0, 32, powerpc56_algorithm_info->context[i]);
...
int powerpc56_run_algorithm(struct target *target, int num_mem_params,
						  ...
						  int timeout_ms, void *arch_info)
{
	...
	err = powerpc56_start_algorithm(target,
	...
	if (err == ERROR_OK)
		err = powerpc56_wait_algorithm(target,
				num_mem_params, mem_params,
				num_reg_params, reg_params,
...
int powerpc56_arch_state_gen(struct target *target)
{
	struct powerpc56_common *powerpc = target_to_powerpc(target);
	if (powerpc->common_magic != POWERPC_COMMON_MAGIC) {
...
static int powerpc56_arch_state(struct target *target)
{
	struct powerpc56_common *powerpc = target_to_powerpc(target);
	int retval;
	...
	retval = powerpc56_arch_state_gen(target);
...
static const struct reg_arch_type powerpc56_reg_type = {
	.get = powerpc56_get_core_reg,
	.set = powerpc56_set_core_reg,
};

struct reg_cache *powerpc56_build_reg_cache(struct target *target)
{
	struct powerpc56_common *powerpc = target_to_powerpc(target);
	...
	struct powerpc56_core_reg *arch_info = malloc(sizeof(struct powerpc56_core_reg) * num_regs);
	...
	for (i = 0; i < num_regs; i++) {
		arch_info[i].num = powerpc56_regs[i].id;
		arch_info[i].target = target;
		arch_info[i].powerpc56_common = powerpc;

		reg_list[i].name = powerpc56_regs[i].name;
		reg_list[i].size = 32;

		reg_list[i].value = calloc(1, 4);
		reg_list[i].valid = 0;
		reg_list[i].type = &powerpc56_reg_type;
		reg_list[i].arch_info = &arch_info[i];

		reg_list[i].reg_data_type = calloc(1, sizeof(struct reg_data_type));
		if (reg_list[i].reg_data_type)
			reg_list[i].reg_data_type->type = powerpc56_regs[i].type;
		else
			LOG_ERROR("unable to allocate reg type list");

		reg_list[i].dirty = 0;

		reg_list[i].group = powerpc56_regs[i].group;
		reg_list[i].number = i;
		...
		if (feature) {
			feature->name = powerpc56_regs[i].feature;
...
static int powerpc56_init_target(struct command_context *cmd_ctx,
	struct target *target)
{
	/* Build register cache */
	powerpc56_build_reg_cache(target);
	return ERROR_OK;
}
 ...
 */
static void powerpc56_deinit_target(struct target *target)
{
	struct powerpc56_common *powerpc = target_to_powerpc(target);

	LOG_DEBUG("target->state: %s",  target_state_name(target));

	powerpc56_once_release(target);
...
static int powerpc56_examine(struct target *target)
{
	uint32_t idcode;
	...
	if (!target_was_examined(target)) {
		...
		if (powerpc56_jtag_get_idcode(target) != idcode)
			return ERROR_FAIL;
	}

	powerpc56_decode_idcode(target->tap->idcode);

	return ERROR_OK;
}

static int powerpc56_target_request_data(struct target *target,
...
static int powerpc56_target_create(struct target *target, Jim_Interp *interp)
{
	struct powerpc56_common *powerpc = calloc(1, sizeof(struct powerpc56_common));
	...
	powerpc->read_core_reg = powerpc56_read_core_reg;
	powerpc->write_core_reg = powerpc56_write_core_reg;
	 ...
	 */
	powerpc56_write_debug_info_bits(target, DEBUG_HALT, 0);
...
COMMAND_HANDLER(powerpc56_setspr)
{
	struct target *target = get_current_target(CMD_CTX);
	struct powerpc56_common *powerpc = target_to_powerpc(target);
	int retval;

	retval = powerpc56_verify_pointer(CMD, powerpc);
	if (retval != ERROR_OK)
		return retval;
	...
	if (CMD_ARGC < 1)
		...
		if (CMD_ARGC == 1) {
			uint32_t value;
			retval = powerpc56_once_spr_read(target, spr_reg, &value);
			if (retval != ERROR_OK) {
		...
		/*Write a SPR register */
			...
			retval = powerpc56_once_spr_write(target, spr_reg, value);
			if (retval != ERROR_OK) {
				command_print(CMD,
...
COMMAND_HANDLER(powerpc56_tlb1_set)
{
	struct target *target = get_current_target(CMD_CTX);
	struct powerpc56_common *powerpc = target_to_powerpc(target);
	int retval;
	...
	if (CMD_ARGC < 4)
		...
		uint32_t mas2;
		uint32_t mas3;
		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[0], mas0);
		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[1], mas1);
		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[2], mas2);
		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[3], mas3);
		retval = powerpc56_once_spr_write(target, SPR_MAS0, mas0);
		if (retval != ERROR_OK) {
			command_print(CMD, "couldn't access spr reg SPR_MAS0");
			return ERROR_OK;
		}
		retval = powerpc56_once_spr_write(target, SPR_MAS1, mas1);
		if (retval != ERROR_OK) {
			command_print(CMD, "couldn't access spr reg SPR_MAS1");
			return ERROR_OK;
		}
		retval = powerpc56_once_spr_write(target, SPR_MAS2, mas2);
		if (retval != ERROR_OK) {
			command_print(CMD, "couldn't access spr reg SPR_MAS2");
			return ERROR_OK;
		}
		retval = powerpc56_once_spr_write(target, SPR_MAS3, mas3);
		if (retval != ERROR_OK) {
			command_print(CMD, "couldn't access spr reg SPR_MAS3");
			return ERROR_OK;
		}
		powerpc56_once_tlbwe(target);
	}
	return ERROR_OK;
}


COMMAND_HANDLER(powerpc56_ram_fill)
{
	struct target *target = get_current_target(CMD_CTX);
	struct powerpc56_common *powerpc = target_to_powerpc(target);
	struct working_area *fill_algorithm;
	struct reg_param reg_params[5];
	struct powerpc56_algorithm powerpc56_info;
	int retval;
	int err;
	unsigned int i;
	...
	/* Set arch info */
	powerpc56_info.common_magic = POWERPC_COMMON_MAGIC;
	...
	/* RAM fill code */
	if (target_alloc_working_area(target, sizeof(spc56x_ram_fill),
	...
	powerpc56_read_memory(target, fill_algorithm->address, 2, fill_algorithm->size / 2, memory_backup);

	err = target_write_buffer(target, fill_algorithm->address,
			sizeof(spc56x_ram_fill), (uint8_t *)spc56x_ram_fill);
	if (err != ERROR_OK) {
	 ...
	 */
	init_reg_param(&reg_params[2], "lr", 32, PARAM_IN);
	buf_set_u32(reg_params[2].value, 0, 32, fill_algorithm->address + (sizeof(spc56x_ram_fill) - 2));

	/*
	* Stack Pointer (in).
	*/
	init_reg_param(&reg_params[3], "r1", 32, PARAM_IN);
	buf_set_u32(reg_params[3].value, 0, 32, target->working_area_phys + target->working_area_size - 1);
	...
	powerpc56_write_memory(target, fill_algorithm->address, 2, fill_algorithm->size / 2, memory_backup);
	...
	err = powerpc56_once_read_cpu_scan_regs(target->tap, &loc_cpu_status);
	if (err != ERROR_OK)
		return err;
...
static const struct command_registration powerpc56_exec_command_handlers[] = {
	{
		.name = "setspr",
		.handler = powerpc56_setspr,
		.mode = COMMAND_EXEC,
		.usage = "regnum [value]",
	...
	{
		.name = "tlb1_set",
		.handler = powerpc56_tlb1_set,
		.mode = COMMAND_EXEC,
		.usage = "mas0 mas1 mas2 mas3",
		.help = "configure tlb1 entry",
		},
...
struct target_type powerpc56_target = {
	...
	.poll = powerpc56_poll,
	.arch_state = powerpc56_arch_state,

	.target_request_data = powerpc56_target_request_data,

	.halt = powerpc56_halt,
	.resume = powerpc56_resume,
	.step = powerpc56_step,

	.assert_reset = powerpc56_assert_reset,
	.deassert_reset = powerpc56_deassert_reset,
	.soft_reset_halt = powerpc56_soft_reset_halt,

	.get_gdb_reg_list = powerpc56_get_gdb_reg_list,

	.read_memory = powerpc56_read_memory,
	.write_memory = powerpc56_write_memory,

	.run_algorithm = powerpc56_run_algorithm,
	.start_algorithm = powerpc56_start_algorithm,
	.wait_algorithm = powerpc56_wait_algorithm,

	.add_breakpoint = powerpc56_add_breakpoint,
	.remove_breakpoint = powerpc56_remove_breakpoint,
	.add_watchpoint = powerpc56_add_watchpoint,
	.remove_watchpoint = powerpc56_remove_watchpoint,

	.target_create = powerpc56_target_create,
	.init_target = powerpc56_init_target,
	.examine = powerpc56_examine,
	.deinit_target = powerpc56_deinit_target,

	.commands = powerpc56_exec_command_handlers,
};
