...
/** */
static int stlink_dap_op_dp_read(struct adiv5_dap *dap, unsigned int reg,
		uint32_t *data)
{
	uint32_t dummy;
	int retval;
...
/** */
static int stlink_dap_op_dp_write(struct adiv5_dap *dap, unsigned int reg,
		uint32_t data)
{
	int retval;
...
/** */
static int stlink_dap_op_ap_read(struct adiv5_ap *ap, unsigned int reg,
		uint32_t *data)
{
...
/** */
static int stlink_dap_op_ap_write(struct adiv5_ap *ap, unsigned int reg,
		uint32_t data)
{
	struct adiv5_dap *dap = ap->dap;
	int retval;
...
static int stlink_dap_op_run(struct adiv5_dap *dap)
{
	...
	if (dap->stlink_flush_ap_write) {
		dap->stlink_flush_ap_write = false;
		retval = stlink_dap_op_dp_read(dap, DP_RDBUFF, NULL);
		if (retval != ERROR_OK) {
			dap->do_reconnect = true;
			return retval;
		}
	...
	retval = stlink_dap_op_dp_read(dap, DP_CTRL_STAT, &ctrlstat);
	if (retval != ERROR_OK) {
		dap->do_reconnect = true;
		return retval;
	}
	...
	if (ctrlstat & SSTICKYERR) {
		if (stlink_dap_handle->st_mode == STLINK_MODE_DEBUG_JTAG)
			retval = stlink_dap_op_dp_write(dap, DP_CTRL_STAT,
					ctrlstat & (dap->dp_ctrl_stat | SSTICKYERR));
		else
			retval = stlink_dap_op_dp_write(dap, DP_ABORT, STKERRCLR);
		if (retval != ERROR_OK) {
			dap->do_reconnect = true;
			return retval;
		}
...
static int stlink_usb_buf_rw_segment(void *handle, const struct dap_queue *q, unsigned int len)
{
	uint32_t bufsize = len * CMD_MEM_AP_2_SIZE(q[0].cmd);
	uint8_t buf[bufsize];
	...
	case CMD_MEM_AP_WRITE32:
		...
		if (len > 1 && q[0].mem_ap.addr == q[1].mem_ap.addr)
...
static int stlink_usb_cnt_misc_rw_queue(void *handle, const struct dap_queue *q, unsigned int len,
...
static int stlink_usb_cnt_buf_rw_queue(const struct dap_queue *q, unsigned int len)
{
...
static int stlink_usb_mem_rw_queue(void *handle, const struct dap_queue *q, unsigned int len, unsigned int *skip)
{
	...
	unsigned int cnt_buf = stlink_usb_cnt_buf_rw_queue(q, len);

	if (cnt_misc > cnt_buf) {
		cnt = cnt_misc;
		retval = stlink_usb_misc_rw_segment(handle, q, cnt, misc_items);
	} else {
		cnt = cnt_buf;
		retval = stlink_usb_buf_rw_segment(handle, q, cnt_buf);
	}
	if (retval != ERROR_OK)
		return retval;
...
static void stlink_dap_op_queue_run_internal(struct adiv5_dap *dap)
{
	...
	while (i && stlink_dap_error == ERROR_OK) {
		unsigned int skip = 1;

		switch (q->cmd) {
		case CMD_DP_READ:
			retval = stlink_dap_op_dp_read(q->dp_r.dap, q->dp_r.reg, q->dp_r.p_data);
			break;
		case CMD_DP_WRITE:
			retval = stlink_dap_op_dp_write(q->dp_w.dap, q->dp_w.reg, q->dp_w.data);
			break;
		case CMD_AP_READ:
			retval = stlink_dap_op_ap_read(q->ap_r.ap, q->ap_r.reg, q->ap_r.p_data);
			break;
		case CMD_AP_WRITE:
			/* ignore increment packed, not supported */
			if (q->ap_w.reg == MEM_AP_REG_CSW)
				q->ap_w.data &= ~CSW_ADDRINC_PACKED;
			retval = stlink_dap_op_ap_write(q->ap_w.ap, q->ap_w.reg, q->ap_w.data);
			break;
...
{
	stlink_dap_op_queue_run_internal(dap);
	return stlink_dap_op_run(dap);
}
	...
	uint32_t *data)
{
	...
	if (i == MAX_QUEUE_DEPTH - 1)
		stlink_dap_op_queue_run_internal(ap->dap);
